...
for fold in range(5):
    ...
    class trDataset(torch.utils.data.Dataset):
        ...
        def __getitem__(self, idx):
            img_path = os.path.join(self.root, r'E:\FASTRCNN\FASTRCNN\dataset2\balanced_dataset\images_trian with limit dot-test with full dot_5pixel',
                                    self.imgs[idx])
...
# ---------------------------- 6️⃣ افزودن برگهٔ Total Samples ----------------------------
import pandas as pd
from pathlib import Path
from openpyxl import load_workbook  # اطمینان از نصب:  pip install openpyxl

base_results_dir = Path(root_dir) / "checkpoints_cross"      #  ...\\checkpoints_cross
result_folders   = [f"result{i}" for i in range(5)]           # result0 … result4

frames = []
for idx, fold_name in enumerate(result_folders):
    csv_path = base_results_dir / fold_name / "total_samples.csv"
    if csv_path.exists():
        df = pd.read_csv(csv_path)
        df["Fold"] = f"fold{idx}"
        # مطمئن شو ستون اسم درست است
        if "Total Samples" not in df.columns and "Total Samples".lower() in map(str.lower, df.columns):
            df.columns = ["Total Samples"]
        frames.append(df[["Fold", "Total Samples"]])

# فقط اگر داده پیدا شد
if frames:
    ...
    avg_row = pd.DataFrame({
        "Fold": ["Average"],
        "Total Samples": [total_df["Total Samples"].mean()]
    })
    total_df = pd.concat([total_df, avg_row], ignore_index=True)

    # الحاق به اکسل (در صورت وجود، شیت را جایگزین می‌کند)
    ...
    print(f"✅ برگهٔ «Total Samples» به {final_report_path} افزوده شد.")
else:
    print("⚠️ هیچ total_samples.csv در فولدها پیدا نشد؛ برگه‌ای اضافه نشد.")


# ---------- تنظیم مسیرهای فولدرهای result ----------
BASE_DIR = r"E:\FASTRCNN\FASTRCNN\dataset2\balanced_dataset\checkpoints_cross"
RESULT_FOLDERS = [f"result{i}" for i in range(5)]   # result0 … result4

# ---------- جمع‌آوری total_samples.csv های هر فولدر ----------
records = []
for folder in RESULT_FOLDERS:
    csv_path = os.path.join(BASE_DIR, folder, "total_samples.csv")
...
if records:
    ...
    out_path = os.path.join(BASE_DIR, "all_total_samples.csv")
